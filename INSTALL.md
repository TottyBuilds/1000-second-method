...
**For Oura, use the Connect Oura procedure in the `/sources` command below during this interview too.** Read `docs/oura.md` from the raw repo base and follow its user walkthrough and agent setup reference. Explain the prerequisites and the operator's three actions: register their own application, save the application credentials locally, and approve access in the browser. Prepare the local callback before asking them to register a redirect URI. Do not send them searching for a token or ask them to paste secrets into chat. Mark Oura connected only after a successful live test; a fixture run is not a connection test.
...
**Connect Oura:**
1. Read the authoritative Oura setup at `https://raw.githubusercontent.com/TottyBuilds/1000-second-method/main/docs/oura.md` and save it as `system/health/oura.md`. Follow **Connect Oura** and **Setup reference for the installing agent** for both install-time and later connections. Confirm Python 3.9+, access to the operator's Oura account, and active membership for Gen3 or later. Explain that the operator registers their own application in `https://developer.ouraring.com/`, saves its credentials locally, and approves browser consent. OAuth2 with `daily` scope is the setup path; do not offer retired PAT creation.
2. Install `system/health/oura_client.py` from `https://raw.githubusercontent.com/TottyBuilds/1000-second-method/main/docs/oura_client.py`. The helper fetches data and refreshes tokens; it does not perform initial authorization. Prepare the guide's one-time local callback and give the operator its exact redirect URI before application registration. Follow consent/state validation and the local code exchange without exposing secrets in chat or command output. Store the exact `oura` keys: `client_id`, `client_secret`, `access_token`, `refresh_token`, `access_token_expires_at` (absolute Unix seconds). Preserve existing Strava credentials.
3. From inside `1000-second-system/`, run `python3 system/health/oura_client.py --credentials system/health/credentials.json --output system/health/health-data.json`. On success add the Oura entry to `system/sources.md`, record the last successful read, and report today's readiness/sleep score or "not yet synced" for missing readings. Tell the operator to run `/render` and open Health. On failure leave Oura setup incomplete, report the error using the guide's troubleshooting section, and continue the install. Offline fixtures do not prove that the operator's account is connected.
